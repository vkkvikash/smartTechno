...
export default function OrderInformation() {
	...
	const { price: total } = usePrice(
		data && {
			amount: parseFloat(data?.shipping_amount?.replace(/,/g, '')) ? parseFloat(data?.total?.replace(/,/g, '')) + parseFloat(data?.shipping_amount?.replace(/,/g, '')) : parseFloat(data?.total.replace(/,/g, '')),
			currencyCode: "INR",
		}
	);
