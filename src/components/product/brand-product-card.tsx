...
const BrandProductCard: FC<ProductProps> = ({
    product,
    className = '',
    contactClassName = '',
    ...
    function handlePopupView() {
        setModalData({
            data: {
                slug: product.slug,
                image: product?.image?.image ?? placeholderImage,
                name: product?.product_name,
                description: product.description,
                sale_price: product.vendor_info.discount_price,
                price: product.vendor_info.product_price
            }
        });
        ...
        >
            ...
            >
                ...
                <div className="absolute top-3.5 md:top-5 3xl:top-7 start-3.5 md:start-5 3xl:start-7 flex flex-col gap-y-1 items-start">
                    {product?.vendor_info?.discount_price && (variant === 'gridModernWide' || variant === 'gridModern' || variant === 'gridTrendy') && (
          ...
          ${variant === 'gridModern' || variant === 'gridModernWide' || variant === 'gridTrendy'
                ...
                >
                    {product?.vendor_info?.price && <span className={`inline-block ${demoVariant === 'ancient' && 'font-bold text-gray-900 text-lg'}`}>{product?.vendor_info?.price}</span>}
                    {product.vendor_info.product_price && <del className={`sm:text-base font-normal ${bgTransparent ? 'text-white/70' : 'text-gray-800'}`}>{product?.vendor_info?.product_price}</del>}
                </div>
            </div>
