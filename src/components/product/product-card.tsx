...
const ProductCard: FC<ProductProps> = ({
  ...
  const { price, basePrice, discount } = usePrice({
    amount: Number(product.sale_price) > 0 ? Number(product.sale_price) : Number(product.price),
  ...
  function handlePopupView() {
    setModalData({
      data: {
        id: product.id,
        slug: product.slug,
        image: product?.image?.thumbnail || placeholderImage,
        name: product?.name,
        description: product.description,
        model: product.model,
        sale_price: Number(product.sale_price) > 0 ? Number(product.sale_price) : Number(product.price),
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3.5 md:top-5 3xl:top-7 start-3.5 md:start-5 3xl:start-7 flex flex-col gap-y-1 items-start">
          {discount && (variant === 'gridModernWide' || variant === 'gridModern' || variant === 'gridTrendy' || variant === 'grid') && (
          ...
          {product?.label && (variant === 'gridModernWide' || variant === 'gridModern' || variant === 'gridTrendy' || variant === 'grid') && (
      ...
      >
        {(variant === 'gridModern' || variant === 'gridModernWide' || variant === 'gridTrendy' || variant === 'grid') && (
          <div className="py-2 flex items-center gap-x-2">
            ...
            <span className="text-xs sm:text-sm text-heading font-semibold truncate">{product.ratings}</span>
        ...
        {!!(showCategory || showRating) && (
          <div className="flex flex-col md:flex-row md:items-center lg:flex-row xl:flex-row 2xl:flex-row  mb-0.5 items-start">
            ...
            {!!showRating && <RatingDisplay rating={Number(product.ratings)} />}
          </div>
        )}
        <h2
        ...
        {!hideProductDescription && product?.model && (
          <p className="text-body text-xs lg:text-sm leading-normal xl:leading-relaxed max-w-[250px] truncate">{product?.model}</p>
        )}
        <div
        ...
        >
          {product && <span className={`inline-block ${demoVariant === 'ancient' && 'font-bold text-gray-900 text-lg'}`}>{price}</span>}
          {product && <del className={`sm:text-base font-normal ${bgTransparent ? 'text-white/70' : 'text-gray-800'}`}>{basePrice}</del>}
        </div>
      </div>

      {(variant === 'gridTrendy' || variant === 'gridModern' || variant === 'grid') && (
