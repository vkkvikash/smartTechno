...
const CartItem: React.FC<CartItemProps> = ({ item, handleReload }) => {
	...
	const addIem = async () => {
		const isAdded = await addItemToCart(item.id, 1);
		if (isAdded.success) {
			handleReload(true)
		}
	}
	const removeIem = async () => {
		const isRemoved = await removeItemFromCart(item.id, item.qty - 1)
