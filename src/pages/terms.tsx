...
export default function TermsPage() {
	...
	const [termsAndServices, setTermsAndServices] = useState<any>([])
	useEffect(() => {
		let isApiSubscribe = true;
		async function getData() {
			const response: any = await submitAPI({}, "GET", "page/terms-and-conditions", {})
			if (response.success) {
				setTermsAndServices(response.data.terms_and_conditions)
			}
		}
		if (isApiSubscribe) {
			getData()
					...
					<div className="flex flex-col md:flex-row">
						<nav className="md:w-72 xl:w-3/12 mb-8 md:mb-0">
							<ol className="sticky md:top-16 lg:top-28 z-10">
								{termsAndServices?.map((item: any, index: any) => (
									<li key={item.id}>
										<Link
											spy={true}
											offset={-160}
